    ...
    public partial class ProductService : IProductService
    {
            ...
            bool? overridePublished = null)
        {
            ...
            if (_commonSettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
            {
               ...
               // products = products.Where(x => x.OfferStatus != "Pending" && x.OfferStatus != "Rejected" && x.OfferStatus != "Expired").ToList();
        ...
        public virtual IPagedList<Product> GetOffers(string ProductName, string ProductStatus, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            if(!string.IsNullOrEmpty(ProductName) && ProductStatus !="Select")
            {
                var query = _productRepository.Table;
                query = query.Where(x => x.IsOffer == true && x.OfferStatus != "Delete" && ProductName.Contains(x.Name) && x.OfferStatus == ProductStatus);
                query = query.Where(x => !x.Deleted);
            ...
            {
                var query = _productRepository.Table;
                query = query.Where(x => x.IsOffer == true && x.OfferStatus != "Delete" && x.Name.Contains(ProductName));
                query = query.Where(x => !x.Deleted);
                query = query.OrderByDescending(x => x.Id);
            ...
            else if (ProductStatus != "Select")
            {
                var query = _productRepository.Table;
                query = query.Where(x => x.IsOffer == true && x.OfferStatus != "Delete" && x.OfferStatus == ProductStatus);
                query = query.Where(x => !x.Deleted);
                query = query.OrderByDescending(x => x.Id);
            ...
            {
                var query = _productRepository.Table;
                query = query.Where(x => x.IsOffer == true && x.OfferStatus != "Delete");
                query = query.Where(x => !x.Deleted);
                query = query.OrderByDescending(x => x.Id);
        ...
        //public virtual IPagedList<Product> OfferSalesList(int pageIndex = 0, int pageSize = int.MaxValue)
        //{
        //    var query = _productRepository.Table;
        //    query = query.Where(x => x.IsOffer == true && (x.OfferStatus == "Order Cancelled by User" || x.OfferStatus == "Order Cancelled by Admin" ||
        //    x.OfferStatus == "Approved" || x.OfferStatus == "Expired" || x.OfferCompleted == true));
        //    query = query.Where(x => !x.Deleted);
        //    query = query.OrderByDescending(x => x.Id);
        ...
        public virtual List<Product> GetAllOffers()
        {
            return _productRepository.Table.Where(x => x.IsOffer == true && x.OfferCompleted != true && !x.Deleted && x.OfferStatus == "Approved").ToList();
        }

        public virtual IQueryable<Product> CounteredOffersList(int CustomerId)
        {
            var query = _productRepository.Table;
            query = query.Where(x => x.IsOffer == true && (x.OfferStatus == "Countered By Admin" || x.OfferStatus == "Approved" ||
            x.OfferStatus == "Rejected") && x.OfferedBy == CustomerId);
            query = query.Where(x => !x.Deleted);
            return query;
        }
    }
