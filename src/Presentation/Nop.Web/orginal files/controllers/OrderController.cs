...
namespace Nop.Web.Controllers
{
    public partial class OrderController : BasePublicController
    {
        ...
        [HttpPost]
        public virtual ActionResult CheckOffersExpiry()
        {
            var products = _productService.GetAllOffers();

            foreach (var item in products)
            {
                var sci = _shoppingCartService.FindItemByProductId(item.Id);
                if (sci != null)
                {
                    _shoppingCartService.DeleteShoppingCartItem(sci, ensureOnlyActiveCheckoutAttributes: true);
                }

                TimeSpan DurationCompleted = DateTime.UtcNow - item.CreatedOnUtc;
                if (DurationCompleted.Days >= 1)
                {
