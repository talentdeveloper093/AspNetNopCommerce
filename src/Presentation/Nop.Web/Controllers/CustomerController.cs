...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BasePublicController
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                switch (loginResult)
                {
                    case CustomerLoginResults.Successful:
                    {
                        ...
                        if(model.IsCart!=null && model.IsCart == "Yes")
                         {
        ...
        [NopHttpsRequirement(SslRequirement.Yes)]
        public virtual ActionResult CounterOffersInfo()
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return new HttpUnauthorizedResult();
            ...
            List<CounterOffersModel> model = _productService.CounteredOffersList(_workContext.CurrentCustomer.Id).Select(x => new CounterOffersModel {
                Id = x.Id,
                Name = x.Name,
                MessageToSeller = x.MessageToSeller,
                Price = x.Price,
                OriginalProductid = x.OriginalProductid,
                OfferStatus = x.OfferStatus
            }).ToList();

            foreach (var item in model)
            {
                item.ActualPrice = FindActualPrice(item.OriginalProductid);
            }
        ...
        [HttpPost]
        public virtual ActionResult SubmitCountForOffer(int ProductId, int Price, string MessageToSeller)
        {
            var product = _productService.GetProductById(ProductId);
            product.OfferStatus = "Countered By User";
            product.Price = Price;
            product.MessageToSeller = MessageToSeller;
        ...
        [HttpPost]
        public virtual ActionResult AcceptOffer(int productId)
        {
            var cartType = ShoppingCartType.ShoppingCart;
            int quantity = 1;
            var newProduct = _productService.GetProductById(productId);
            ...
            var product = _productService.GetProductById(newProduct.Id);
            product.OfferStatus = "Approved";
            product.MessageToSeller = "Approved by user";
